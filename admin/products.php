...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" class="well">
        ...
        <div class="row">
            ...
            <div class="col-md-4"><input class="form-control" name="name" placeholder="Название" value="<?= admin_h((string)($edit['name'] ?? '')) ?>" required></div>
            <div class="col-md-2"><input class="form-control" name="price" placeholder="Цена" value="<?= admin_h((string)($edit['price'] ?? '0')) ?>" required></div>
            <div class="col-md-2"><input class="form-control" name="old_price" placeholder="Старая цена" value="<?= admin_h((string)($edit['old_price'] ?? '0')) ?>"></div>
        </div>
        <div class="row" style="margin-top:10px;">
            <div class="col-md-4"><input class="form-control" name="link" placeholder="/slug" value="<?= admin_h((string)($edit['link'] ?? '')) ?>" required></div>
            <div class="col-md-4"><input class="form-control" name="image" placeholder="Путь к фото" value="<?= admin_h((string)($edit['image'] ?? '')) ?>"></div>
            <div class="col-md-4"><input type="file" class="form-control" name="image_upload"></div>
        </div>
        <div class="row" style="margin-top:10px;">
            <div class="col-md-6"><input class="form-control" name="seo_title" placeholder="SEO title" value="<?= admin_h((string)($edit['seo_title'] ?? '')) ?>"></div>
            <div class="col-md-6"><input class="form-control" name="seo_description" placeholder="SEO description" value="<?= admin_h((string)($edit['seo_description'] ?? '')) ?>"></div>
        </div>
        <div class="row" style="margin-top:10px;">
            <div class="col-md-12"><textarea class="form-control" rows="2" name="short_desc" placeholder="Краткое описание"><?= admin_h((string)($edit['short_desc'] ?? '')) ?></textarea></div>
        </div>
        <div class="row" style="margin-top:10px;">
            <div class="col-md-6"><textarea class="form-control" rows="4" name="desc" placeholder="Описание"><?= admin_h((string)($edit['desc'] ?? '')) ?></textarea></div>
            <div class="col-md-6"><textarea class="form-control" rows="4" name="full_desc" placeholder="Полное описание"><?= admin_h((string)($edit['full_desc'] ?? '')) ?></textarea></div>
        </div>
        <div style="margin-top:10px;">
            <label><input type="checkbox" name="in_stock" value="1" <?= !empty($edit['in_stock']) ? 'checked' : '' ?>> В наличии</label>
            <input class="form-control" style="display:inline-block; width:220px; margin-left:10px;" name="status" placeholder="status (preorder)" value="<?= admin_h((string)($edit['status'] ?? '')) ?>">
            <button class="btn btn-success" style="margin-left:10px;"><?= $edit ? 'Сохранить' : 'Добавить товар' ?></button>
        </div>
    </form>

    <table class="table table-bordered table-striped">
        <thead><tr><th>ID</th><th>Slug</th><th>Название</th><th>Цена</th><th>SEO</th><th></th></tr></thead>
        <tbody>
